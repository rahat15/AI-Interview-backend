#!/usr/bin/env python3
"""
Test script to verify LLM integration for CV evaluation
"""
...
def test_llm_integration():
    """Test LLM integration with mock API keys"""
...
"""
    ...
    print("🧪 Testing LLM Integration for CV Evaluation")
    print("=" * 60)
    
    # Test 1: Without API keys (should use heuristic)
    print("\n1️⃣ Testing without Groq API key (heuristic fallback):")
    engine_no_llm = CVEvaluationEngine(use_llm=True)
    ...
    result = engine_no_llm.evaluate(request)
    ...
    if engine_no_llm.use_llm and engine_no_llm.llm_scorer:
        print("   ✅ Groq scorer initialized")
    else:
        print("   🔧 Using heuristic scoring (no Groq API key)")
    
    # Test 2: Force heuristic mode
    print("\n2️⃣ Testing with heuristic mode only:")
    engine_heuristic = CVEvaluationEngine(use_llm=False)
    
    result = engine_heuristic.evaluate(request)
    print(f"   Fit Index: {result.fit_index}/100 ({result.band})")
    print(f"   CV Quality: {result.cv_quality.overall_score}/100")
    print(f"   JD Match: {result.jd_match.overall_score}/100")
    print("   🔧 Heuristic scoring confirmed")
    
    # Test 3: Test with mock API keys (simulate LLM success)
    print("\n3️⃣ Testing with mock API keys:")
    ...
    try:
        # This will fail to initialize LLM scorer but should fall back gracefully
    ...
    print("   2. Run: python test_evaluation.py")
    print("   3. The system will automatically use Groq scoring if available")
